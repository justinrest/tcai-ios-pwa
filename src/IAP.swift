...
//        }
    ...
    func purchaseProduct(productID: String, quantity: Int, userUUID: String?) async throws {
        print("Function called with productID: \(productID), quantity: \(quantity), userUUID: \(userUUID ?? "nil")")
...
func returnProductsResult(jsonString: String){
    DispatchQueue.main.async(execute: {
        CaribbeanAI.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('iap-products-result', { detail: '\(jsonString)' }))")
    })
}

func returnPurchaseResult(state: String){
    DispatchQueue.main.async(execute: {
        CaribbeanAI.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('iap-purchase-result', { detail: '\(state)' }))")
    })
}
func returnPurchaseTransaction(jsonString: String){
    DispatchQueue.main.async(execute: {
        CaribbeanAI.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('iap-purchase-transaction', { detail: '\(jsonString)' }))")
    })
}

func returnActiveTransactions(jsonString: String){
    DispatchQueue.main.async(execute: {
        CaribbeanAI.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('iap-transactions-result', { detail: '\(jsonString)' }))")
    })
}
